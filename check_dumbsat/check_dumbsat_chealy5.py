...
def make_plot(my_wffs):
    ...
    # I am pretty sure Dumbsat may be exponential so I am going to do that for the trendline
    def exponential_func(x, a, b):
        return a * np.exp(b * x)

    # Fit the exponential to the data
    params, covariance = curve_fit(exponential_func, n_literals, time_to_solve)

    # Get the x and y values for the scatterplot
    x = np.linspace(min(n_literals), max(n_literals), 100)
    y = exponential_func(x_fit, *params)
